...
List* DelListNode(List *list, Node *n)
{
    ListNode *lnode = NULL;
    ...
    for(prenode = lnode = list->head; lnode != NULL; lnode = lnode->next)
    {
        if(lnode->value == n)
            break;
...
Node* MergeListNode(Node *lresult, Node *rresult)
{
    List *head = NULL;
    Node *left = NULL;
    ...
    else if(lresult->type == T_List)
    {
        head = (List*)lresult;
        left = rresult;
    }
    else if(rresult->type == T_List)
    {
        head = (List*)rresult;
        left = lresult;
    }
    ...
    {
        head = (List*)AppendList(head, lresult);
        head = (List*)AppendList(head, rresult);
    }
    ...
    if(left != NULL)
    {
        if(left->type == T_List)
        {
            ListNode *listNode = ((List*)left)->head;
            for(; listNode != NULL; listNode = listNode->next)
            {
                head = (List*)AppendList(head, listNode->value);
            }
        }
        else 
        {
            head = (List*)AppendList(head, left);
        }
    }
