...
Node* MergeResult(Node *lresult, Node *rresult)
{
    List *head = NULL;
    Node *left = NULL;

    if(lresult->type == T_List)
    {
        head = (List*)lresult;
        left = rresult;
    }
...
Node* ExecGeneralNullResult(Node *node)
{
    ...
    switch(node->type)
    {
        ...
        case T_NestLoopNode:
        {
            ...
            result = MergeResult(lresult, rresult);
        }
        break;
        default:
        break;
        ...
        for( ; ; )
        {
            ...
            { 
                if(nlNode->joinType & JOIN_LEFT)
                {
                    if(!nlNode->outerOnce)
                    {
                        resultList = (List*)MergeResult(lresult, ExecGeneralNullResult(nlNode->rightplan));
