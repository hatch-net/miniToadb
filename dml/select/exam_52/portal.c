...
int ShowDataByTypes(char *buf, int bufOffset, int dataType, int maxWidth, ColumnDataHeader *colData);
...
void PortalShowResult(Node *result, Node *targetList, int showHead)
{
    ...
    for(lnode = ((List*)targetList)->head; lnode != NULL; lnode = lnode->next)
    {
        ...
        width = ShowDataByTypes(buf, bufOffset, te->attr->attrType, te->attr->maxWidth, colData);
...
int ShowDataByTypes(char *buf, int bufOffset, int dataType, int maxWidth, ColumnDataHeader *colData)
{
    ...
    if(colData->size == NULL_COLUMN_SIZE)
    {
        snprintf(buf + bufOffset, PORT_BUFFER_SIZE - bufOffset, "%-*s|", maxWidth, "Null");
        return colDataOffset;
    }
