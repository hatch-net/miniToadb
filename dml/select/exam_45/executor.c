...
{
    int ret = 0, rowNum = 0;
    SelectStmt *stmt = (SelectStmt*)rootNode;
    ScanInfo scanInfo = {0};

    List *fromClause = (List *)stmt->fromList;
    TableRefInfo *tblRefInfo = (TableRefInfo *)fromClause->head->value;
    ColumnName *refTable = (ColumnName *)tblRefInfo->val;

    TupleHeader *tup = NULL;
    int showHead = 1;

    scanInfo.rel = OpenRelation(refTable->sub_name);
    if(scanInfo.rel == NULL)
    {
        return -1;
    }

    scanInfo.item_offset = PAGE_ITEM_START;
    scanInfo.pageNum = PAGE_NUM_START;
    ...
    do {        
        ...
        // seqscan 
        tup = SeqScanNext(&scanInfo);
        if(tup == NULL)
            break;

        // show 
        PortalShowRowData(scanInfo.rel, tup, showHead);
        if(showHead)
            showHead = 0;
